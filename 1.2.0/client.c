...
case 5:
char ELReader[50000];
bzero(ELReader,sizeof(ELReader));
recv(cd,ELReader,sizeof(ELReader),0);
usleep(10);
printf("%s",ELReader);
usleep(5);
break;
...
case 1:
char loanReader[50000];
bzero(loanReader,sizeof(loanReader));
recv(cd,loanReader,sizeof(loanReader),0);
usleep(10);
printf("%s",loanReader);
usleep(5);
break;
...
printf("Enter the customer username: ");
scanf("%s",l_username);
printf("Whom to assign to:  ");
scanf("%s",el_username);
send(cd,l_username,strlen(l_username),0);
recv(cd,lack,sizeof(lack),0);
send(cd,el_username,strlen(el_username),0);
int lact_check;
recv(cd,&lact_check,sizeof(int),0);
if(lact_check==1){ printf("Task Completed\n");}
else printf("Failed to change details\n");
break;
