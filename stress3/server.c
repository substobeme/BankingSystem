...
int check_tcust(struct USER *loginUser,char *r_username)
{
    struct USER User,User2;;
    int ans=0;
    int ans1=0;
    int fd=open("user.db", O_RDONLY);
    int fd2=open("user.db", O_RDONLY);
    while (read(fd, &User, sizeof(struct USER)) > 0)
    {   
        if (strcmp(loginUser->username, User.username) == 0 && User.role==3 && User.status==1)
        {   printf("Found sender\n");
            ans = 1;
        }
        
    }
    while (read(fd2, &User2, sizeof(struct USER)) > 0)
    {   
        if (strcmp(r_username, User2.username) == 0 && User2.role==3 && User2.status==1)
...
int authenticate(struct USER *loginUser)
{
    ...
    {
        if (strcmp(loginUser->username, User.username) == 0 && loginUser->role == User.role && User.status == 1)
        {   
            lock.l_type = F_WRLCK;      
            lock.l_whence = SEEK_SET;   
            lock.l_start = soff;       
...
int transfer(struct USER *loginUser,char *receiver, float amount)
{
    ...
    int t_check= check_tcust(loginUser,receiver);
    if (t_check==0){ return 1; }
